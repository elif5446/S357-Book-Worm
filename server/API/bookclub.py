...
def get_club_progress(club_id: str, authorization: Optional[str] = Header(None)):
    ...
    user_id = get_user_id_from_token(supabase, token)
    if not user_id:
        raise HTTPException(status_code=401, detail="Invalid token")

    try:
        rows = (
            supabase.table('MemberProgress')
            .select("current_pages, goal_pages")
            .eq("club_id", club_id)
            .eq("user_id", user_id)
            .limit(1)
            .execute()
        ...
        if rows:
            return ReadingProgressOut(
                currentPages=rows[0]['current_pages'],
                goalPages=rows[0].get('goal_pages', 3300),
                clubId=club_id
            )
        # First time — upsert a 0-page row so the member shows up
        supabase.table('MemberProgress').upsert({
            "club_id": club_id,
            "user_id": user_id,
            "current_pages": 0,
            "goal_pages": 3300
...
def update_club_progress(
    ...
    try:
        # Single upsert — no need to check if row exists first
        supabase.table('MemberProgress').upsert({
            "club_id": club_id,
            "user_id": user_id,
            "current_pages": body.currentPages,
...
def get_all_members_progress(club_id: str, authorization: Optional[str] = Header(None)):
    ...
    try:
        # Get all members of the club in one query
        members = (
            supabase.table('ClubMembers')
            .select("user_id")
            .eq("club_id", club_id)
        ...
        # Get all progress rows in one query
        progress_rows = (
            supabase.table('MemberProgress')
            .select("user_id, current_pages, goal_pages")
            .eq("club_id", club_id)
            .execute()
            .data
        )
        ...
        # Merge — every member appears, even if they haven't updated yet
        result = []
        for uid in member_ids:
            prog = progress_map.get(uid)
            result.append(MemberProgressOut(
